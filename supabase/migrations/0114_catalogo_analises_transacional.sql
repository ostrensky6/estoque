...
-- Autorizacao: mesma regra de escrita das tabelas (0112): coordenador+ ou
...
do $$
begin
  if to_regprocedure('kontrol_private.tem_permissao(text)') is null then
    raise exception '0114: requer a migration 0112 (kontrol_private.tem_permissao)';
  end if;
end $$;
...
set search_path = ''
as $$
  select public.papel_minimo('coordenador') or kontrol_private.tem_permissao('analises.editar');
$$;
