...
--     categoria_saida = 'descarte' (coluna criada aqui, se ainda nao existir);
...
begin
  if to_regprocedure('public.descartar_lote(bigint, text)') is null then
    raise exception '0115: requer a funcao descartar_lote(bigint, text) (0014)';
  end if;
end $$;

-- Categoria estruturada da saída (descarte, perda, vencimento...); nula nas
-- movimentações antigas e nas saídas comuns.
