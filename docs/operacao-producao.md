...
- O slug antigo `ostrenskys-projects-17ce406b` era de outra conta, e tratado como scope proibido pelos scripts de producao.
...
- Deploy: manual via `npm run prod:deploy`, com `VERCEL_TOKEN` definido no ambiente. O script valida `.vercel/project.json`, bloqueia scopes proibidos, usa `vercel --prod --token $env:VERCEL_TOKEN --scope ostrensky-s-projects` e depois fixa explicitamente o alias `kontrol-gia.vercel.app`.
