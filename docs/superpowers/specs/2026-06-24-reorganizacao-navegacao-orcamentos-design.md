...
| **Parâmetros econômicos** | Defaults globais de margens/impostos/fundos. Também aparece como etapa na proposta, mas o destino global permanece. |
...
Stepper horizontal no topo da proposta aberta. Etapas exibidas conforme a
`modalidade` da demanda:

- **Apenas análises:** Dados da demanda → Análises laboratoriais → Parâmetros econômicos → Proposta final
- **Apenas projeto:** Dados da demanda → Custos do projeto → Parâmetros econômicos → Proposta final
- **Projeto + análises:** Dados da demanda → Análises laboratoriais → Custos do projeto → Parâmetros econômicos → Proposta final
